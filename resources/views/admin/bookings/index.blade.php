...
@php
    $is_active = function ($button_selection, $current_selection) {
        return $button_selection === $current_selection
            ...
            <div class="mt-3 sm:mt-4">
                <div class="sm:hidden">
                    ...
                    <select id="current-tab" name="current-tab"
                        ...
                        <option value="{{ route('admin.bookings.index', ['selection' => 'all']) }}"
                            {{ $selection == 'all' ? 'selected' : '' }}>All</option>
                        <option value="{{ route('admin.bookings.index', ['selection' => 'pending']) }}"
                            {{ $selection == 'pending' ? 'selected' : '' }}>Pending Confirmation</option>
                        <option value="{{ route('admin.bookings.index', ['selection' => 'confirmed']) }}"
                            {{ $selection == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                        <option value="{{ route('admin.bookings.index', ['selection' => 'in_transit']) }}"
                            {{ $selection == 'in_transit' ? 'selected' : '' }}>In Transit</option>
                        <option value="{{ route('admin.bookings.index', ['selection' => 'completed']) }}"
                            {{ $selection == 'completed' ? 'selected' : '' }}>Completed</option>
                        <option value="{{ route('admin.bookings.index', ['selection' => 'cancelled']) }}"
                            {{ $selection == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                    </select>
                </div>
                <div class="hidden sm:block">
                    <nav class="-mb-px flex space-x-8">
                        <a href="{{ route('admin.bookings.index', ['selection' => 'all']) }}"
                            class="{{ $is_active('all', $selection ?? 'all') }} whitespace-nowrap border-b-2 px-1 pb-4 text-sm font-medium">All</a>
                        <a href="{{ route('admin.bookings.index', ['selection' => 'pending']) }}"
                            class="{{ $is_active('pending', $selection ?? 'all') }} whitespace-nowrap border-b-2 px-1 pb-4 text-sm font-medium">Pending</a>
                        <a href="{{ route('admin.bookings.index', ['selection' => 'confirmed']) }}"
                            class="{{ $is_active('confirmed', $selection ?? 'all') }} whitespace-nowrap border-b-2 px-1 pb-4 text-sm font-medium">Confirmed</a>
                        <a href="{{ route('admin.bookings.index', ['selection' => 'in_transit']) }}"
                            class="{{ $is_active('in_transit', $selection ?? 'all') }} whitespace-nowrap border-b-2 px-1 pb-4 text-sm font-medium">In
                            Transit</a>
                        <a href="{{ route('admin.bookings.index', ['selection' => 'completed']) }}"
                            class="{{ $is_active('completed', $selection ?? 'all') }} whitespace-nowrap border-b-2 px-1 pb-4 text-sm font-medium">Completed</a>
                        <a href="{{ route('admin.bookings.index', ['selection' => 'cancelled']) }}"
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                            <div class="flex items-center">
                                                <div
                                                    class="h-10 w-10 flex-shrink-0 flex items-center justify-center rounded-full bg-indigo-100">
                                                    <i class="fas fa-truck text-indigo-600"></i>
                                                </div>
                                                <div class="ml-4">
                                                    <div class="font-medium text-gray-900">#{{ $booking->id }}</div>
                                                    <div class="text-gray-500">{{ $booking->truck->plate_number ?? 'N/A' }}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-medium text-gray-900">{{ $booking->origin ?? 'From: ' . ($booking->initial_latitude ?? 'N/A') . ', ' . ($booking->initial_longitude ?? 'N/A') }}</span>
                                                <span class="text-xs text-gray-400">to</span>
                                                <span
                                                    class="font-medium text-gray-900">{{ $booking->destination ?? 'To: ' . ($booking->destination_latitude ?? 'N/A') . ', ' . ($booking->destination_longitude ?? 'N/A') }}</span>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-medium text-gray-900">{{ $booking->customer->name ?? 'N/A' }}</span>
                                                <span
                                                    class="text-xs text-gray-500">{{ $booking->customer->email ?? '' }}</span>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-medium text-gray-900">{{ $booking->transporter->name ?? ($booking->truck->transporter_name ?? 'N/A') }}</span>
                                                <span
                                                    class="text-xs text-gray-500">{{ $booking->truck->truck_type ?? '' }}</span>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            @php
                                        ...
                                        <td
                                            ...
                                            @if ($booking->order_status === 'pending')
                                                <form
                                                    action="{{ route('admin.bookings.submit', ['selection' => $selection]) }}"
                                            ...
                                            @elseif($booking->order_status === 'in_transit')
                                                ...
                                                <form
                                                    ...
                                                    <input type="hidden" name="booking_id" value="{{ $booking->id }}">
                                                    <button type="submit" class="text-red-600 hover:text-red-900">
                                                        <span class="sr-only">Cancel in-transit booking
                                                            #{{ $booking->id }}</span>
                                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg"
                                                            viewBox="0 0 20 20" fill="currentColor">
                                                            <path
                                    ...
                                    <tr>
                                        <td colspan="7"
                                            ...
                                            <div class="flex flex-col items-center justify-center py-12">
                                                ...
                                                <p class="mt-1 text-sm text-gray-500">No truck bookings found for this
                                                    status.</p>
                                            </div>
                                        </td>
                                    </tr>
