...
class DashboardController extends Controller
{
    ...
    {
        $activeLoadsCount = Load::whereIn('status', ['pending', 'assigned', 'in_transit'])->count();
        $bookingsToday = Booking::whereDate('created_at', Carbon::now()->toDateString())->count();
        $deliveredThisMonth = Booking::where('status', 'completed')
            ->whereMonth('completed_at', Carbon::now()->month)
            ->whereYear('completed_at', Carbon::now()->year)
            ->count();
        $activeDrivers = Driver::where('status', 'active')->count();
        $activeTransporters = Transporter::where('status', 'active')->count();
        // Active trucks with tracking
        $activeTrucks = Truck::where('status', 'active')
            ->whereNotNull('tracking_data')
            ->get();

        // KYC items pending approval
        $pendingKyc = KycVerification::where('status', 'pending')->latest()->take(10)->get();
     ...
     */
    public function bookings(string $selection)
    {
        // try {
     ...
     */
    public function bookings_submit(Request $request, string $selection)
    {
        // try {
        //     if (!$this->firestore) {
     ...
     */
    public function loadApprovals(string $selection)
    {
        // try {
     ...
     */
    public function load_bookings(string $selection)
    {
        // try {
     ...
     */
    public function kycDetail(string $id)
    {
        // try {
        //     $trucksRef = $this->firestore->collection('trucks');
     ...
     */
    public function load_owners(Request $request)
    {
        // try {
     ...
     */
    public function loads(string $selection = 'all')
    {
        // try {
        //     $loadsRef = $this->firestore->collection('loads');
        //     $query = $loadsRef->orderBy('added_date', 'desc');
     ...
     */
    public function loadDetail(string $selection, string $id)
    {
        // try {
        //     $loadsRef = $this->firestore->collection('loads');
     ...
     */
    public function loadBookingDetail(string $selection, string $id)
    {
        // try {
        //     $loadOrdersRef = $this->firestore->collection('load_orders');
     ...
     */
    public function truckDetail(string $id)
    {
        // try {
        //     // Get truck data
     ...
     */
    public function trucks(string $selection = 'all')
    {
        // try {
        //     $trucksRef = $this->firestore->collection('trucks');
